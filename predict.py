...
def build_prompt(tokenizer, instruction, text, device):
    ...
    prompt_id = im_start_tokens + nl_tokens + prompt_id + nl_tokens + tokenizer.encoder("assistant",allowed_special=set()) + nl_tokens
    input_ids = torch.tensor([input_ids]).to(device)
    return input_ids

def predict_one_sample(model, tokenizer, instruction, text, args):
    # 获取解码的配置参数，涉及生成内容最大长度，TopP解码和Top-P概率、温度、重复惩罚因子等
    ...
    # 根据文本内容，融合提示词和输入对话内容，构建模型输入所需要的input-ids
    input_ids = build_prompt(tokenizer, instruction, text, model.device)
    # 进行结果预测
    outputs = model.generate(input_ids, generation_config=generation_config, stop_word_ids=[[tokenizer.im_end_id], [tokenizer.im_start_id]])
    # 仅取生成内容
...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="/root/auto-fs/DiaExtra/output_dir_qlora/epoch_1", help="")
...
if __name__ == '__main__':
    ...
    # 输入对话文本， 进行对话要素抽取
    while True:
        print('开始对对话内容进行要素抽取，输入ctrl+c退出')
        text = input("输入的对话内容为：")
